...
const IS_PROD =
  process.env.NODE_ENV?.toString().toLowerCase().trim() === "production";

// Server-to-Server app type credentials from https://marketplace.zoom.us
const ZOOM_REQUIRED_CREDENTIALS = [
  "ZOOM_ACCOUNT_ID",
  "ZOOM_CLIENT_ID",
  "ZOOM_CLIENT_SECRET",
];
...
const generateEnvironment = () => {
  ZOOM_REQUIRED_CREDENTIALS.forEach(
    (cred) =>
      (INPUT_SCHEMA.properties[cred] = {
        required: true,
        default: process.env[cred],
      })
  );
  ...
  prompt.get(INPUT_SCHEMA, (err, input) => {
    const envCredentials = Object.keys(input)
      .map((key) => `\n${key}=${input[key]}`)
      .join("");
...
(() => {
  ...
  try {
    ...
    const requiredCredentials = Object.keys(ZOOM_CREDENTIALS).every(
      (el) => !!ZOOM_CREDENTIALS[el]
    );
